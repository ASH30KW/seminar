...
def checkParams(model):
    ...
    if n_params > PARAM_LIMIT:
        print(
            "# Paramters: The model has {} mio. params but must have less than 5 mio. params. Simplify the model before submitting it. You won't need that many params :)".format(
            bcolors.colorize(color, normalized)))
        return False
    print("# Paramters: The model has {} mio. params.".format(bcolors.colorize(color, normalized)))
    return True
...
def test_and_save(model, val_dataloader, test_dataloader):
    ...
    if val_acc < ACC_THRESHOLD:
        print(f"{bcolors.colorize('red', 'That is too low!')} Please tune the model in order to reach at least {bcolors.colorize('yellow', ACC_THRESHOLD * 100)}% before running on the test set and submitting!")
        return
